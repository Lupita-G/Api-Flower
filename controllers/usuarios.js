...
function modificarUsuario(req, res, next) {
  ...
  Usuario.findById(req.usuario.id).then(user => {
    ...
    user.save().then(updatedUser => {                 //Guardando datos actualizados de usuario, modificado en MongoDB.
...
function iniciarSesion(req, res, next) { // se requiere email y password
  if (!req.body.email) {
